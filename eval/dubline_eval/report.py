...
def write_summary(bundle: Path) -> None:
    ...
    lines = [f"# Eval run {run['run_id']}", "",
             ...
             "| clip | job | utt | wall s | QC | lip-sync | dead air s | unmuted src s | take overlap s | default audio | mouth-on-silence total s | pic offset out/in (frames) | boundary jump max | sharpness | bed RMS Δ dB | bed ratio Δ dB | bed missing | src speech w/o dub s | master squash dB | PSNR outside edit (med/min) | PSNR unedited (med/min) |",
             "|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|"]
    for c in clips:
